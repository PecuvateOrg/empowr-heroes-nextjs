...
- **✅ 2026-08-29 — design-audit fixes live on `main`.** `/design-audit` run against 6 public routes × 8 viewports found 1 HIGH + ~41 MEDIUM findings. Fixed and merged: `.btn`/`.tca` buttons now guarantee 44px tap targets (were 38-42px on mobile); `--muted` and `--patron-gold` design tokens darkened for WCAG AA contrast (`--patron-gold-text` consolidated as an alias of `--patron-gold` since both needs converged on one value). **Still open, not fixed:** HIGH — `/contact` form fields (company/name/email/topic/message) have no visible focus indicator at any viewport. MEDIUM — missing `<h1>` on `/become`/`/projects`/`/tiers`; nav never sets `aria-current="page"`; 2 badges render below the 12px text floor (`.fp-badge`, `.patron-hero-badge`/`.patron-eyebrow`, both 10.88px); one unresolved ~2.8:1 contrast near-miss (single sample per page, unidentified element). LOW — 5 `/contact` fields trigger iOS zoom-on-focus; `/patron` skips a heading level (h1→h3).
- **📧 2026-08-29 (finding only, no code changed): email templates carry real brand drift.** `src/core/email-template.js` is **956 lines holding 9 separate `<!DOCTYPE` shells**, and its palette mixes brand tokens with off-brand ones — notably **`#4f6ef7`, a different blue from brand `#4A70C2`**, in live donor email. Heroes does have **plain-text variants** Members lacks (`buildEmailText`, `buildOneTimeEmailText`) — preserve them if a shared shell is ever adopted. Shared shell is **proposed, NOT approved**; see `Empowr CIC/DEVLOG.md` 2026-08-29. Treat as logged, not scheduled.
