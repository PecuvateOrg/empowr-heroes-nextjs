...
- **But that config now looks like a live bug, evidenced by real donors.** If all 6 links land on `/thankyou`, the **4 one-time donors** (Apr–Jun 2026) were shown *"You're now officially an Empowr Hero"* with three steps promising a Hero badge — while `donation-handler.js` correctly sent them the one-time email, which has no badge. `/thankyou/onetime` exists, matches that email, and nothing routes to it. This is no longer theoretical: 4 real people hit it. Re-point the one-time Payment Link at `/thankyou/onetime` — supersedes the earlier "do not re-raise" note, which assumed nobody had ever completed the flow.
